...
fn main() {
  ...
  if max_seq_len <= u8::MAX as usize {
    multi_threaded_centroid_estimator::<u8>(
      &mut thread_pool,
      &fasta_records,
      output_dir_path,
      use_contra_model,
    ...
    multi_threaded_centroid_estimator::<u16>(
      &mut thread_pool,
      &fasta_records,
      output_dir_path,
      use_contra_model,
...
fn multi_threaded_centroid_estimator<T>(
...
fn compute_and_write_mea_sss<T>(
  ...
  T: HashIndex,
{
  ...
  for (rna_id, fasta_record) in fasta_records.iter().enumerate() {
    ...
    let buf_4_rna_id = format!(">{}\n", rna_id)
      ...
      + if rna_id < num_of_fasta_records - 1 {
        "\n"
      } else {
        ""
      };
