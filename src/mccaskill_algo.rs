...
pub fn get_bpp_and_unpair_prob_mats(seq: SeqSlice) -> (ProbMat, Probs) {
  let seq_len = seq.len();
  // let log_ss_ppf_mats = get_log_ss_ppf_mats(&seq[..], seq_len);
  let bpp_mat = mccaskill_algo(&seq[..]);
  ...
  for sub_seq_len in MIN_SPAN_OF_INDEX_PAIR_CLOSING_HL .. seq_len + 1 {
    for i in 0 .. seq_len - sub_seq_len + 1 {
      ...
      if pp_closing_loop.1 - pp_closing_loop.0 + 1 >= MIN_SPAN_OF_INDEX_PAIR_CLOSING_HL && is_canonical(&bp_closing_loop) {
        let new_term = get_hl_fe(seq, &pp_closing_loop) as FreeEnergy;
        logsumexp(&mut sum, new_term);
        for k in i + 1 .. j - 1 {
          for l in k + 1 .. j {
            ...
            /* if log_ss_ppf_4_base_pairing.is_finite() {
              let ep_of_term_4_log_pf = log_ss_ppf_4_base_pairing + get_2_loop_fe(seq, &pp_closing_loop, &accessible_pp) as FreeEnergy;
            ...
            let new_term = log_ss_ppf_4_base_pairing + get_2_loop_fe(seq, &pp_closing_loop, &accessible_pp) as FreeEnergy;
            logsumexp(&mut sum, new_term);
          }
        }
        for k in i + 1 .. j {
          /* let ep_of_term_4_log_pf = log_ss_ppf_mats.log_ss_ppf_mat_4_at_least_1_base_pairings_on_mls[i + 1][k - 1] + log_ss_ppf_mats.log_ss_ppf_mat_4_rightmost_base_pairings[k][j - 1] + (CONST_4_INIT_ML_DELTA_FE + COEFFICIENT_4_TERM_OF_NUM_OF_BRANCHING_HELICES_ON_INIT_ML_DELTA_FE + ML_TM_DELTA_FES[&(invert_bp(&bp_closing_loop), invert_bp(&(seq[i + 1], seq[j - 1])))] + if is_au_or_gu(&bp_closing_loop) {HELIX_AU_OR_GU_END_PENALTY_DELTA_FE} else {0.});
          if ep_of_term_4_log_pf.is_finite() {
            if max_ep_of_term_4_log_pf < ep_of_term_4_log_pf {max_ep_of_term_4_log_pf = ep_of_term_4_log_pf;}
            eps_of_terms_4_log_pf.push(ep_of_term_4_log_pf);
          } */
          let new_term = log_ss_ppf_mats.log_ss_ppf_mat_4_at_least_1_base_pairings_on_mls[i + 1][k - 1] + log_ss_ppf_mats.log_ss_ppf_mat_4_rightmost_base_pairings[k][j - 1] + (CONST_4_INIT_ML_DELTA_FE + COEFFICIENT_4_TERM_OF_NUM_OF_BRANCHING_HELICES_ON_INIT_ML_DELTA_FE + ML_TM_DELTA_FES[&(invert_bp(&bp_closing_loop), invert_bp(&(seq[i + 1], seq[j - 1])))] + if is_au_or_gu(&bp_closing_loop) {HELIX_AU_OR_GU_END_PENALTY_DELTA_FE} else {0.});
          logsumexp(&mut sum, new_term);
        }
      }
      ...
      log_ss_ppf_mats.log_ss_ppf_mat_4_base_pairings[pp_closing_loop.0][pp_closing_loop.1] = sum;
      /* let mut eps_of_terms_4_log_pf = EpsOfTerms4LogPf::new();
      let mut max_ep_of_term_4_log_pf = NEG_INFINITY; */
      sum = NEG_INFINITY;
      for k in i + 1 .. j + 1 {
        ...
        /* let ep_of_term_4_log_pf = log_ss_pf_4_bp + (if i > 0 && k < seq_len - 1 {
        ...
        let new_term = log_ss_pf_4_bp + (if i > 0 && k < seq_len - 1 {
          ML_TM_DELTA_FES[&(accessible_bp, (seq[i - 1], seq[k + 1]))]
        } else if i > 0 {
          FIVE_PRIME_DE_DELTA_FES[&(accessible_bp, seq[i - 1])]
        } else if k < seq_len - 1 {
      ...
      log_ss_ppf_mats.log_ss_ppf_mat_4_rightmost_base_pairings[i][j] = sum;
      sum = 0.;
      /* let mut eps_of_terms_4_log_pf = EpsOfTerms4LogPf::new();
      ...
      for k in i .. j {
        ...
        if log_ss_ppf_4_rightmost_base_pairings.is_finite() {
          ...
          let new_term = if i == 0 && k == 0 {0.} else {log_ss_ppf_mats.log_ss_ppf_mat[i][k - 1]} + log_ss_ppf_4_rightmost_base_pairings;
          logsumexp(&mut sum, new_term);
        }
      }
      ...
      log_ss_ppf_mats.log_ss_ppf_mat[i][j] = sum;
      sum = log_ss_ppf_mats.log_ss_ppf_mat_4_rightmost_base_pairings[i][j] + COEFFICIENT_4_TERM_OF_NUM_OF_BRANCHING_HELICES_ON_INIT_ML_DELTA_FE;
      /* let mut eps_of_terms_4_log_pf = EpsOfTerms4LogPf::new();
      let mut max_ep_of_term_4_log_pf = log_ss_ppf_mats.log_ss_ppf_mat_4_rightmost_base_pairings[i][j] + COEFFICIENT_4_TERM_OF_NUM_OF_BRANCHING_HELICES_ON_INIT_ML_DELTA_FE;
      if max_ep_of_term_4_log_pf.is_finite() {
        eps_of_terms_4_log_pf.push(max_ep_of_term_4_log_pf);
      } */
      for k in i + 1 .. j {
        let log_ss_ppf_4_rightmost_base_pairings = log_ss_ppf_mats.log_ss_ppf_mat_4_rightmost_base_pairings[k][j];
        /* let ep_of_term_4_log_pf = log_ss_ppf_4_rightmost_base_pairings + COEFFICIENT_4_TERM_OF_NUM_OF_BRANCHING_HELICES_ON_INIT_ML_DELTA_FE;
        if ep_of_term_4_log_pf.is_finite() {
          if max_ep_of_term_4_log_pf < ep_of_term_4_log_pf {max_ep_of_term_4_log_pf = ep_of_term_4_log_pf;}
          eps_of_terms_4_log_pf.push(ep_of_term_4_log_pf);
        } */
        let new_term = log_ss_ppf_4_rightmost_base_pairings + COEFFICIENT_4_TERM_OF_NUM_OF_BRANCHING_HELICES_ON_INIT_ML_DELTA_FE;
        logsumexp(&mut sum, new_term);
        /* let ep_of_term_4_log_pf = log_ss_ppf_mats.log_ss_ppf_mat_4_at_least_1_base_pairings_on_mls[i][k - 1] + log_ss_ppf_4_rightmost_base_pairings + COEFFICIENT_4_TERM_OF_NUM_OF_BRANCHING_HELICES_ON_INIT_ML_DELTA_FE;
        if ep_of_term_4_log_pf.is_finite() {
          if max_ep_of_term_4_log_pf < ep_of_term_4_log_pf {max_ep_of_term_4_log_pf = ep_of_term_4_log_pf;}
          eps_of_terms_4_log_pf.push(ep_of_term_4_log_pf);
        } */
        let new_term = log_ss_ppf_mats.log_ss_ppf_mat_4_at_least_1_base_pairings_on_mls[i][k - 1] + log_ss_ppf_4_rightmost_base_pairings + COEFFICIENT_4_TERM_OF_NUM_OF_BRANCHING_HELICES_ON_INIT_ML_DELTA_FE;
        logsumexp(&mut sum, new_term);
      }
      /* if eps_of_terms_4_log_pf.len() > 0 {
      ...
      log_ss_ppf_mats.log_ss_ppf_mat_4_at_least_1_base_pairings_on_mls[i][j] = sum;
    }
  }
  log_ss_ppf_mats
}
    ...
    for i in 0 .. seq_len - sub_seq_len + 1 {
      ...
      for k in j + 1 .. seq_len {
        ...
        if log_ss_ppf_4_base_pairing.is_finite() {
          ...
          let log_coefficient = log_bpp + (ml_tm_delta_fe + if is_au_or_gu(&bp_closing_loop) {HELIX_AU_OR_GU_END_PENALTY_DELTA_FE} else {0.});
          ...
          let new_term = log_coefficient - log_ss_ppf_4_base_pairing + log_ss_ppf_mats.log_ss_ppf_mat_4_at_least_1_base_pairings_on_mls[j + 1][pp_closing_loop.1 - 1];
          logsumexp(&mut sum_1, new_term);
          ...
          let new_term = log_coefficient - log_ss_ppf_4_base_pairing;
          logsumexp(&mut sum_2, new_term);
        }
      }
      ...
      log_prob_mat_4_mls_1[i][j] = sum_1;
      log_prob_mat_4_mls_2[i][j] = sum_2;
      ...
      /* let mut eps_of_terms_4_log_prob = EpsOfTerms4LogProb::new();
      let mut max_ep_of_term_4_log_prob = if accessible_pp.0 < 1 {0.} else {log_ss_ppf_mats.log_ss_ppf_mat[0][accessible_pp.0 - 1]} + log_ss_ppf_4_base_pairing_1 + if accessible_pp.1 > seq_len - 2 {0.} else {log_ss_ppf_mats.log_ss_ppf_mat[accessible_pp.1 + 1][seq_len - 1]} - log_ss_ppf + (if i > 0 && j < seq_len - 1 {
      ...
      let mut sum = if accessible_pp.0 < 1 {0.} else {log_ss_ppf_mats.log_ss_ppf_mat[0][accessible_pp.0 - 1]} + log_ss_ppf_4_base_pairing_1 + if accessible_pp.1 > seq_len - 2 {0.} else {log_ss_ppf_mats.log_ss_ppf_mat[accessible_pp.1 + 1][seq_len - 1]} - log_ss_ppf + (if i > 0 && j < seq_len - 1 {
        ML_TM_DELTA_FES[&(accessible_bp, (seq[i - 1], seq[j + 1]))]
      } else if i > 0 {
        FIVE_PRIME_DE_DELTA_FES[&(accessible_bp, seq[i - 1])]
      } else if j < seq_len - 1 {
      ...
      for k in 0 .. i {
        for l in j + 1 .. seq_len {
          ...
          if log_ss_ppf_4_base_pairing_2.is_finite() {
            /* let ep_of_term_4_log_prob = log_bpp_mat[pp_closing_loop.0][pp_closing_loop.1] + log_ss_ppf_4_base_pairing_1 - log_ss_ppf_4_base_pairing_2 + get_2_loop_fe(seq, &pp_closing_loop, &accessible_pp) as FreeEnergy;
            if ep_of_term_4_log_prob.is_finite() {
              if max_ep_of_term_4_log_prob < ep_of_term_4_log_prob {max_ep_of_term_4_log_prob = ep_of_term_4_log_prob;}
              eps_of_terms_4_log_prob.push(ep_of_term_4_log_prob);
            } */
            let new_term = log_bpp_mat[pp_closing_loop.0][pp_closing_loop.1] + log_ss_ppf_4_base_pairing_1 - log_ss_ppf_4_base_pairing_2 + get_2_loop_fe(seq, &pp_closing_loop, &accessible_pp) as FreeEnergy;
            logsumexp(&mut sum, new_term);
          }
        }
      }
      let log_coefficient = log_ss_ppf_4_base_pairing_1 + (CONST_4_INIT_ML_DELTA_FE + COEFFICIENT_4_TERM_OF_NUM_OF_BRANCHING_HELICES_ON_INIT_ML_DELTA_FE + if i > 0 && j < seq_len - 1 {
        ML_TM_DELTA_FES[&(accessible_bp, (seq[i - 1], seq[j + 1]))]
      } else if i > 0 {
        FIVE_PRIME_DE_DELTA_FES[&(accessible_bp, seq[i - 1])]
      } else if j < seq_len - 1 {
      ...
      for k in 0 .. i {
        ...
        let new_term = log_coefficient + log_prob_mat_4_mls_2[k][j] + log_ss_ppf_4_at_least_1_base_pairings_on_mls;
        logsumexp(&mut sum, new_term);
        ...
        let new_term = log_coefficient + log_prob_4_mls;
        logsumexp(&mut sum, new_term);
        ...
        let new_term = log_coefficient + log_prob_4_mls + log_ss_ppf_4_at_least_1_base_pairings_on_mls;
        logsumexp(&mut sum, new_term);
      }
      ...
      log_bpp_mat[accessible_pp.0][accessible_pp.1] = sum;
    }
  }
  log_bpp_mat
}
